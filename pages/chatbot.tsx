...
export default function Chatbot() {
  ...
  // =========== send GPT response to ---> translate to user selected language =======================
  async function handleTranslation() {
    ...
    if (apiMessageFinal) {
      ...
      const responseTranslate = await fetch(
        'http://solutions.it-marketing.website:4200/translate-to-language',
        {
          method: 'POST',
          headers: {
  ...
  // =========== check and get live agent response =======================
  useEffect(() => {

    if (liveAgent === true) {
      const interval = setInterval(async () => {
        const response = await fetch('http://solutions.it-marketing.website:4200/live-chat-agent', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  // =========== check and get live agent response end =======================
  ...
  async function handleSubmit(e: any) {
    if (liveAgent === false) {
      ...
      // set user message array as 'userMessage'
      setMessageState((state) => ({
        ...state,
        messages: [
          ...state.messages,
      ...
      //send user question --> translate to english
      const response = await fetch('http://solutions.it-marketing.website:4200/translate-to-english', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // send user message to GPT api end point
      try {
  ...
  // =========== send user message to live agent =======================
  const handleLiveAgent = async (e: any) => {
    ...
    let question = query.trim();
    if (liveAgent === true) {
      const response = await fetch('http://solutions.it-marketing.website:4200/live-chat-user', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  // =========== send user message to live agent end =======================
  ...
  // =========== switch chat to live agent =======================
  const SwitchToLiveAgent = async () => {
    const response = await fetch('http://solutions.it-marketing.website:4200/switch-to-live-agent', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async function sendRateValues() {
    try {
      const response = await fetch('http://solutions.it-marketing.website:4200/save-rating', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
